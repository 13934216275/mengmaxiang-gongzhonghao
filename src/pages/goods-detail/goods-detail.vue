...
export default {
  ...
  created() {
    ...
    this.getCartCount()
    this.jiage(this.$route.query.id)
  },
  mounted() {
  ...
  methods: {
    ...
    onClickKanjia() {
      ...
      this.$request.post('/shop/goods/kanjia/join', { token: storage.get('token'), kjid }).then(res => {
        if (res.code !== 0) {
          this.$toast({ message: res.message || '发起砍价失败，请重试！', duration: 1500 })
        }
        this.$toast.clear()
    ...
    getGoodsReputation(goodsId, page = 1, pageSize = 50) {
      this.$request.get('/shop/goods/detail', { goodsId }).then(res => {
        // console.log(res)
        if (res.code !== 0) {
          // this.$toast(res.msg)
          return
    ...
    getDiscountsCoupons() {
      this.$request.get('/home/recommendProductList', { token: storage.get('token') }).then(res => {
        // console.log(res)
        const data = res.data || []
      ...
      this.$request.get('/shop/goods/detail', { id }).then(res => {
        ...
        const marketingFn = () => {
          // if (this.basicInfo.kanjia && this.basicInfo.kanjiaPrice) {
            if (this.basicInfo && this.basicInfo.kanjiaPrice) {
