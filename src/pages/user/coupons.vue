      ...
      <div class="list" v-if="list.length">
        <div class="list-item" v-for="item in list" :key="item.id">
          <div class="list-item-hd">
            <div class="list-item-hd-money">￥<span class="fz24">{{item.amount}}</span></div>
            <div class="list-item-hd-moneyHreshold">满{{item.minPoint}}元可用</div>
          </div>
          <div class="list-item-bd">
            <div class="list-item-bd-inner">
              <div class="list-item-bd-title">{{item.note}}</div>
              <div class="list-item-bd-txt">{{item.startTime | formatDate}} - {{item.endTime | formatDate}}</div>
            </div>
...
export default {
  ...
  data() {
    return {
      tabs: [
        ...
        { name: '已使用', status: '1', buttonTxt: '已使用' },
  ...
  methods: {
    ...
    getMyDiscounts(status) {
      ...
      this.$request.get('/member/coupon/list', { token: storage.get('token'),useStatus:status }).then(res => {
