...
<!--        <img class="user-avatar" src="http://macro-oss.oss-cn-shenzhen.aliyuncs.com/mall/images/20180615/xiaomi.jpg" alt="">-->
      </div>
...
<!--        马志刚-->
    ...
    <div class="main">
      ...
      <div class="tui" @click="tc" v-if="hiddenLogin">
        <button>退出登录</button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getUserInfo() {
      ...
      this.$request.post('/sso/show',{ token: util.storage.get('token')}).then(res => {
    ...
    tc(){
      ...
      // util.sessionStorage.remove('token')
