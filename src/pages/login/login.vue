    ...
    <van-tabs v-model="current" type="card" class="my-tab" >
      <van-tab title="登录">
        ...
        <van-field v-model="pwd"
          ...
          placeholder="请输入密码" />
        <div class="btn-area">
          <van-button type="danger" round size="large" @click="formSubmit">确定登录
          </van-button>
        </div>
      </van-tab>
...
export default {
  ...
  methods: {
    getWxUrl(){
      this.$request.get('/weChat/login',{ token: util.storage.get('token')}).then(res => {
        this.weixinurl = res.data
          // alert(this.weixinurl)
          if (this.weixinurl!=null && this.weixinurl!='') {
              // window.open(this.weixinurl)
              var obj = document.getElementById('links')
              obj.href = this.weixinurl
              obj.click()
          }
      })
    },
    ...
    mobileRegister() {
      ...
      this.$request.post('/sso/register', params).then(res => {
        if (res.code !== 200) {
        ...
        this.$route.query.redirect ? this.$router.replace({ path: this.$route.query.redirect }) : this.$router.replace({ path: '/index/' })
        this.$toast.clear()
        this.$toast.success({
          message: '注册成功',
          duration: 1500
    ...
    // 登录
     mobileLogin() {
      ...
      const params = {
       ...
       this.$request.post('/sso/login', params).then(res => {
        // console.log(res)
        if (res.code == 200) {
          util.storage.set('token', res.data.token)
            let getAuthen = false
            //判断当前用户是否已授权
            this.$request.get('/sso/wxAuthen',{token:res.data.token}).then(res=>{
                if (res.code==200) {
                    getAuthen = res.data
                    if (getAuthen) {
                        this.$route.query.redirect ? this.$router.replace({ path: this.$route.query.redirect }) : this.$router.replace({ path: '/index/' })
